...
bool CommFUNC::BaseInit()
{
    ...
    {
        ros_interface.ROSLog(LogLevel::kError,"xnode_comm: All communication interface is closed,please enable one");
        ros_interface.ROSLog(LogLevel::kError,"xnode_comm: Node close");
        return false;
    }
    if(ros_interface.m_com_en && !m_comm_com.COMIsOpen())ComInit();
    if(ros_interface.m_tcp_en && !m_comm_tcp.TCPIsOpen())TcpInit();
    ros_interface.ROSLog(LogLevel::kInfo,"\033[1;32m xnode_comm: ### communication init finish ### \033[0m");
    return true;
}
/*------------------------------------------------------------------------------------------------------------------
 * name: DataCheckSum
 ...
 * name: ComInit
 -----------------------------------------------------------------------------------------------------------------*/
void CommFUNC::ComInit()
{
    if(m_comm_com.COMIsOpen())return;
    ROSInterface& ros_interface = ROSInterface::GetInterface();
    while(!m_comm_com.COMInitDefault() && ros_interface.Ok())sleep(3);
    m_device_list.clear();
}
/*------------------------------------------------------------------------------------------------------------------
 * name: TcpInit
 -----------------------------------------------------------------------------------------------------------------*/
void CommFUNC::TcpInit()
{
    if(m_comm_tcp.TCPIsOpen())return;
    ROSInterface& ros_interface = ROSInterface::GetInterface();
    while(!m_comm_tcp.TCPOpen() && ros_interface.Ok())sleep(3);
    m_device_list.clear();
}
/*------------------------------------------------------------------------------------------------------------------
 * name: ComSendData
...
unsigned long CommFUNC::ComRecvData()
{
    ...
    {
        no_data_count++;
        if(no_data_count == 1000)ros_interface.ROSLog(LogLevel::kInfo,"\033[5;33m xnode_comm: COM received no data,check cable connect?\033[0m");
        return 0;
    }
    no_data_count = 0;
    ...
    if(ros_interface.m_com_xstd_en)
    {
      ...
      for(i=0;i<com_data_recv.size();i++)
      {
          ...
          if(DataCheckSum(com_data_recv.data()+i) != com_data_recv.at(i + com_data_recv.at(i+1)-1))   //sum cheak
          {
              ...
              data_err_count++;
              ros_interface.ROSLog(LogLevel::kError," xnode_comm: COM receive data error No.%d",data_err_count);
      ...
      if(com_data_recv.size()>0)com_data_recv.erase(com_data_recv.begin(),com_data_recv.begin()+i);
    }
    ...
    if(ros_interface.m_tcp_xstd_en)
    {
      ...
      for(i=0;i<tcp_data_recv.size();i++)
      {
          ...
          if(DataCheckSum(tcp_data_recv.data()+i) != tcp_data_recv.at(i + tcp_data_recv.at(i+1)-1))   //sum cheak
          {
              ...
              data_err_count++;
              ros_interface.ROSLog(LogLevel::kError," xnode_comm: TCP receive data error No.%d",data_err_count);
              if(data_err_count>10000)data_err_count = 0;
              continue;
          }
      ...
      if(tcp_data_recv.size()>0)tcp_data_recv.erase(tcp_data_recv.begin(),tcp_data_recv.begin()+i);
...
void CommFUNC::PubDevState()
{
    ...
    if(m_dev_list_en)
    {
        ros_interface.ROSLog(LogLevel::kInfo,"\033[1;44;30m xnode_comm: ====== New device online ====== \033[0m");
        for(unsigned long i=0;i<m_device_list.size();i++)
        {
